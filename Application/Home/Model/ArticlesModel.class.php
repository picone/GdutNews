...
class ArticlesModel extends Model {
    ...
    protected $fields=array('ArticleID','Title','Publisher','CreateDate','Updator','UpdateDate','CategoryID','DepartmentID','KeyWord','Abstract','Content','PublishDate','ExpireDate','Approved','CommentsEnabled','ReadingPower','ViewCount','Vote','TotalRating','ImageUrl','oldSysID','CategoryID2');
    ...
    public function getLatest($id,$time){
        ...
        if(!$data){
            $data=$this->fetchSql(true)->query('SELECT TOP 5 [ArticleID],[Title] FROM [Articles] WHERE CategoryID=%d AND PublishDate>%s ORDER BY [PublishDate] DESC',(int)$id,$time);
    ...
    public function getTitle($category,$category2,$page){
        ...
        if(!$data){
            ...
            $data=array();
            foreach($tmp as &$val){
                $data[$val['publishdate'].' '.$val['weekday']][]=array('id'=>$val['articleid'],'t'=>$val['title'],'d'=>$val['departmentname']);
            }
